...
picard.ext <- "_hisat2_align.sam.picard.rnaseq.stats"
...
```{r load_hisat_stats, echo=FALSE, results="asis", fig.width=15, fig.height=10}
...
# write.table(align.stats, file=figure_path("align_stats.tsv"), sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE) # something wrong with the write location
...
```{r tpm_fpkm_density, echo=TRUE, fig.width=11,fig.height=8, message=FALSE}

get_featCount_files <- function() {
  system(paste("find", getwd(), "-iname '*sam.counts' | sort"), intern=TRUE)
}
...
```{r write_count, echo=FALSE, results="asis"}
...
load_star <- function(star.dir=getwd()) {
    star.files <- system(paste0("find ", getwd(), " -iname '*.sam.counts' | sort"), intern=TRUE)
