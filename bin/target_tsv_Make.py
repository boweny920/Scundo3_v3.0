...
def fetch_sample_table_from_LIMS(fc_id, molng):
    ...
    run_info = requests.get(
        f"{NGS_LIMS}/flowcells/{fc_id}/samples", headers=header, verify=False
    )
    if not run_info.ok:
...
parser.add_argument(
    ...
    "--output_dir",
    default="./",
    help="Output directory, default=cwd, default='./'",
)
...
parser.add_argument(
    ...
    "--molng",
    default=False,
    help="provide molng id, this can be just empty if the FCID has only 1 MOLNG-ID",
)
...
bardcodes = [
    "-".join(s["sequence"] for s in v) for v in df_sample_info["indexes"].to_list()
]  # This is to consider the situations where two indexes are available
